...
class CopyMaterialRequestsFromInvoiceApi(APIView):
    def post(self,request,*args, **kwargs):
        context={}
        log=11
        context['result']=FAILED
        if request.method=='POST':
            log=22
            report_form=CopyMaterialRequestsFromInvoiceForm(request.POST)
            if report_form.is_valid():
                log=33
                cd=report_form.cleaned_data
                material_requests=MaterialRequestRepo(request=request).copy_material_requests(**cd)
...
class CopyServiceRequestsFromInvoiceApi(APIView):
    def post(self,request,*args, **kwargs):
        context={}
        log=11
        context['result']=FAILED
        if request.method=='POST':
            log=22
            report_form=CopyServiceRequestsFromInvoiceForm(request.POST)
            if report_form.is_valid():
                log=33
                cd=report_form.cleaned_data
