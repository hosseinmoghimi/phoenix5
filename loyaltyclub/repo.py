...
class OrderRepo():  
    ...
    def add_invoice_to_order(self,*args, **kwargs):
        result=FAILED
        message=""
        order=None 

        if not self.request.user.has_perm(APP_NAME+".change_order"):
        ...
        if order is not None:
            result=SUCCEED
            message="فاکتور جدید با موفقیت افزوده شد."
        return result,message,order
    ...
    def sum(self, *args, **kwargs):
        ...
        for order in orders:
            ...
            paids+=order.paid
            ship_fees+=order.ship_fee

        return sum,discounts,paids,ship_fees
...
#             cash_paid_sum+=orders.paid
#             coupon_paid_sum+=orders.discount

#         return cash_paid_sum,coupon_paid_sum
